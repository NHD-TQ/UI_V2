        ...
        with gr.Column(elem_id=f"{id_part}_prompt_container", scale=2):
            with gr.Row():
                with gr.Column(scale=80):
                    with gr.Row():
                        prompt = gr.Textbox(label="Prompt", elem_id=f"{id_part}_prompt", show_label=False, lines=3, placeholder="Enter Prompt", elem_classes=["prompt"])
...
########################
    ...
    (img2txt_interface, "Img2Txt", "img2txt"),
    (Lora_interface, "Train Lora", "Lora"),
    ...
    inter += script_callbacks.ui_tabs_callback()
    inter += [(settings.interface, "Settings", "settings")]

    extensions_interface = ui_extensions.create_ui()
    inter += [(extensions_interface, "Extensions", "extensions")]

    shared.tab_names = []
    for _interface, label, _ifid in inter:
    ...
    with gr.Blocks(theme=shared.gradio_theme, analytics_enabled=False, title="Stable Diffusion") as demo:
        ...
        with gr.Tabs(elem_id="tabs") as tabs:
            ...
            sorted_inter = sorted(inter, key=lambda x: tab_order.get(x[1], 9999))

            for interface, label, ifid in sorted_inter:
                if label in shared.opts.hidden_tabs:
                    continue
                with gr.TabItem(label, id=ifid, elem_id=f"tab_{ifid}"):
                    interface.render()

            for interface, _label, ifid in inter:
